...
![Kiwi Edge AI Stack — hybrid liquid glass architecture](assets/architecture_overview.png)
...
### Clone + manual install (same on both platforms after venv)
...
| Area | What shipped |
| :--- | :--- |
| **Hybrid foundation** | Core · Weaver · Aether · stack diagrams and docs aligned; Aether companion + computer use on Windows/Linux |
| **Installers** | `install.sh` + `install.ps1` on Core, Weaver, Aether, coastal-alpine-stack; Weaver `bootstrap.py` cross-platform |
| **Landing page** | Portfolio platforms column; hybrid Mermaid + architecture image; full Windows/Linux install section |
...
| **Security (org-wide)** | Least-privilege Actions, Dependabot, SECURITY / notifications sections |
| **coastal-alpine-stack** | Multi-plane system map; Mermaid maps in README + `ARCHITECTURE.md` |
| **Aether** | Kiwi Edge skills, computer-use hybrid, dual installers |

**Next focus (suggested):** keep submodules on Core ≥0.5.4; Dependabot PRs; smoke-test installers on a clean Windows and Linux host.
