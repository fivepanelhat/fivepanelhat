...
def require_python_310() -> None:
 ...
 print(
 f"[X] Python 3.10+ required (found {sys.version.split()[0]}). "
...
def run_cmd(cmd, description=None, critical: bool = True) -> bool:
 ...
 print(f" [X] Command failed: {args if isinstance(cmd, list) else cmd}")
 if e.stderr:
 for line in e.stderr.strip().split("\n")[-12:]:
 print(f" {line}")
 if critical:
 print(" [X] Install cannot continue (critical step failed).")
 sys.exit(1)
 return False
...
def create_venv(venv_path: str) -> bool:
 ...
 print(f" [OK] Virtual environment '{venv_path}' already exists")
 return True
