...
THEME_ALIASES = {
 "maori": ["maori", "maori", "iwi", "hapu", "hapu", "whenua", "te mana", "indigenous"],
...
def score_row(row: dict[str, str], proj_themes: set[str], project: str) -> tuple[int, list[str]]:
 ...
 p = project.lower()
 if any(x in p for x in ("maori", "maori", "iwi", "whenua")) and "maori" in tags:
 score += 8
 ...
 # penalties
 if "maori" in tags and not any(x in p for x in ("maori", "maori", "iwi", "whenua", "partner")):
 score -= 12
